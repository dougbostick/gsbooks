...
const categories = props.categories.map(category => {
    const firstThreeProducts = products.filter(product => product.categoryId === category.id).splice(0, 3)
     return (
    <Grid item xs={5} key={category.id}>
        <Card classes={classes.category}>
            <CardContent>
                  <Typography gutterBottom variant="h5" component="h2" color="primary" className={classes.categoryName}>
                    {category.name}
                  </Typography>
            </CardContent>
          ...
          <CardActionArea className={classes.productContainer}>
            <Grid container spacing={2} justifyContent='flex-start'>
                ...
                //the plan is to have the first 3 book images thats in that category displayed, when clicked will link to that specific book.
                    return (
                        <Grid item xs={3} key={product.id} style={{paddingLeft: '1.5rem'}}>
          ...
          <CardActions className={classes.categoryButton}>
            <Button size="small" color="primary">
                <Link to={`/categories/${category.id}`}> 
                    View All ({products.filter(item => item.categoryId === category.id).length})
                </Link>
            </Button>
          </CardActions>
        </Card>
 ...
 return (
     <>
     <div className={classes.title}> <h1> Categories </h1> </div>
